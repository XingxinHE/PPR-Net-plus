...
def load_obj(file_name):
    with open(file_name, 'r') as f:
        ...
        for line in f.readlines():
            line = line.strip()
            if line.startswith('v '): # Check for 'v ' to avoid vn or vt
                words = line.split()[1:]
                xyz = [float(w) for w in words]
                vertexes.append(xyz)
            if line.startswith('f '):
                words = line.split()[1:]
                # Handle face formats like v/vt/vn
                indices = [int(w.split('/')[0]) for w in words]
...
if __name__ == "__main__":
    ...
    print(f"Loaded {vertexes.shape[0]} vertices and {faces.shape[0]} faces")
